  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 p-3 rounded-xl bg-muted/40 border-2 border-dashed border-border">
        {wordBank.map((w, i) => (
          <span key={i} className="px-3 py-1 rounded-full bg-primary/10 text-primary font-body text-lg font-medium border border-primary/20">
            {w}
          </span>
        ))}
      </div>
      {sentences.map((s, i) => {
        ...
        return (
          <motion.div
            ...
            className={`flex flex-wrap items-center gap-1 p-3 rounded-xl border-2 font-body text-lg ${
              showResults
          ...
          >
            ...
            <select
              ...
              className="px-2 py-1 rounded-lg border-2 bg-background font-body text-lg min-w-[140px] focus:ring-2 focus:ring-accent focus:outline-none"
            >
            ...
            {showResults && (
              answers[i]?.toLowerCase().trim() === s.blank.toLowerCase().trim()
                ...
                : <span className="text-lg text-primary font-medium ml-1">({s.blank})</span>
            )}
          </motion.div>
        );
      })}
