...
const Index = () => {
  ...
  const { isRegistered, register } = useRespondent();
  ...
  const handleQuestClick = useCallback(
    (questNumber: number) => {
      if (isRegistered) {
        navigate(`/quest/${questNumber}`);
      } else {
        setPendingQuest(questNumber);
        setShowModal(true);
      }
    },
  ...
  const handleRegistrationSuccess = useCallback(() => {
    setShowModal(false);
    if (pendingQuest) {
      navigate(`/quest/${pendingQuest}`);
      setPendingQuest(null);
    }
  }, [pendingQuest, navigate]);
          ...
          >
            {quests.map((q) => (
              <motion.div key={q.number} variants={item}>
                <div
                  onClick={() => handleQuestClick(q.number)}
