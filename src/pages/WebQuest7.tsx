...
function TaskStep() {
  return (<>
    <p className="font-body text-foreground text-xl mb-3">
    ...
    <div className="grid sm:grid-cols-2 gap-2">
      {["Research the hero's life, achievements, and cultural significance",
        ...
        <Card key={i} className="border-accent/20"><CardContent className="p-3 flex items-start gap-2">
          <span className="flex-shrink-0 w-6 h-6 rounded-full bg-accent/15 text-accent flex items-center justify-center text-lg font-bold">{i + 1}</span>
          <p className="font-body text-xl">{task}</p>
        </CardContent></Card>
      ))}
    </div>
  </>);
...
function CreateStep() {
  return (<>
    <p className="font-body text-foreground text-xl mb-3">Based on your research, create one of the following products:</p>
    <div className="grid sm:grid-cols-2 gap-2 mb-4">
      {[{ title: "🎤 Role-Play Interview", desc: "Prepare and perform an interview with your cultural hero (3-5 min)" },
        ...
        <Card key={i} className="hover:shadow-lg transition-shadow"><CardContent className="p-3">
          <h5 className="font-display font-semibold text-xl mb-1">{p.title}</h5>
          <p className="font-body text-lg text-foreground">{p.desc}</p>
        </CardContent></Card>
      ))}
    </div>
  ...
  return (<>
    ...
    <Card className="mt-4 border-primary/30 bg-primary/5">
      <CardContent className="p-4 text-center">
        ...
        <p className="font-body text-lg text-foreground">Through this WebQuest, you explored the life and legacy of a cultural hero, gaining insight into how individual contributions shape national identity and global culture.</p>
      </CardContent>
    </Card>
  </>);
}
